...
async def get_employees(telegram_id: int):
    async with async_session() as session:
...
async def set_employee(telegram_id: int, company_id: int, start_date_work: str) -> Union[Employee, int]:
    async with async_session() as session:
        # Проверяем, есть ли уже сотрудник с таким Telegram ID и в этой компании
        ...
        # Если сотрудник не найден, добавляем нового
        if not employee:
            ...
            # Фиксируем изменения
            await session.commit()
            await session.refresh(new_employee)  # Обновляем объект новыми данными из базы (например, ID)

            return new_employee  # Возвращаем созданного сотрудника
...
async def get_payouts(employee_id: int = None, company_id: int = None):
    async with async_session() as session:
