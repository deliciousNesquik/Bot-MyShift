from app.database.requests import get_employee, get_company_by_id, get_payouts
...
message_menu_employee_company = "<b>Компании</b>\n\nЗдесь отображаются компании в которых Вы работаете."
...
message_button_show_all_payout = "Просмотреть все выплаты"
...
invalid_company_name = "Пожалуйста, введите корректное название компании."
invalid_company_address = "Пожалуйста, введите правильный адрес компании."
invalid_company_type_of_activity = "Пожалуйста, введите корректный вид деятельности компании."
invalid_manager_name = "Пожалуйста, введите корректное имя"
invalid_manager_phone = "Пожалуйста, введите корректный номер телефона (11 цифр)."
invalid_manager_post = "Пожалуйста, введите корректную должность."
...
message_use_keyboard = "Для навигации по данной компании используйте кнопки на вашей клавиатуре!"
...
async def message_payments(company_id: int):
    message: str = "<b>Операции по сотрудникам</b>\n\n"

    payments = await get_payouts(
        company_id=company_id
    )
    try:
        company = await get_company_by_id(company_id=company_id)
        for payment in payments:
            try:
                employee = await get_employee(
                    employee_id=payment.employee_id
                )
                message += (f"Операция №{payment.id} <b>{payment.date}</b>\n"
                            f"Индефикатор сотрудника\n"
                            f"└<b>{employee.id}</b>\n"
                            f"Фамилия Имя Отчество\n"
                            f"└<b>{employee.full_name}</b>\n"
                            f"Индефикатор компании\n"
                            f"└<b>{payment.company_id}</b>\n"
                            f"Адрес компании\n"
...
async def message_send_employee(from_who: str = "компании", text: str = None):
    return ("<b>Уведомление</b>\n"
            f"Новое сообщение от {from_who}:\n\n"
            f"{text}"
            )
